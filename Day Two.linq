...
foreach (string[] val in values.Select( x => x.Split(' '))) {

	var range = val[0].Split('-').Select(x => int.Parse(x)).ToArray();
	var letter = val[1][0];
	var occurances = val[2].Count(x => x == letter);
	
	// Part 1 is Range
	if (range[0] <= occurances && occurances <= range[1]) {
		++times;
	}
	
	// Part 2 is Exclusive OR
	if (val[2][range[0] - 1] == letter ^ val[2][range[1] - 1] == letter) {
		++partTwoTimes;
	}
}
Console.WriteLine($"Part 1: {times}");
Console.WriteLine($"Part 2: {partTwoTimes}");