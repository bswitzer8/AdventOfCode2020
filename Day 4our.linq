...
var newData = values.Replace("\n\r", "|").Split('|');	
	...
	var validFields = new Dictionary<string, Func<string, bool>>(){
		{"byr", (string s) => { 
		...
		{"hgt", (string s) => {
			var height = int.Parse(Regex.Match(s, @"\d+").Value);	
			return (s.Contains("cm") && height >= 150 && height <= 193) || (s.Contains("in") && height >= 59 && height <= 76);
		} }, //(Height)
		{"hcl", (string s) => new Regex("#[a-f0-9]{6}").Match(s).Success }, //(Hair Color)
		{"ecl", (string s) => "amb blu brn gry grn hzl oth".IndexOf(s) > -1 }, //(Eye Color)
		{"pid", (string s) => new Regex("^[0-9]{9}$").Match(s).Success }, //(Passport ID)
		//{"cid ", (string s) => true }, //(Country ID)
	};
	long partOne = 0, partTwo = 0;
	foreach (var row in data.Select(x => x.Replace("\n", " ").Split(' ') )){
		var rowFormatted = row.Select(x => new [] { x.Split(':').First().Trim(), x.Split(':').Skip(1).First().Trim()});
		
		var validKeys = validFields.Keys.AsEnumerable().Except(rowFormatted.Select(x => x[0]));
		if (validKeys.Count() == 0) {
			++partOne;
		
			var validRes = true;
			foreach(string key in validFields.Keys) {
				var ste = rowFormatted.FirstOrDefault(x => x[0] == key);
				if (ste == null || (ste.Contains(key) && !validFields[ste[0]](ste[1]))) {
					validRes = false;
					break;
				}
			}
