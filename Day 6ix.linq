...
void Main()
{
	var values = GetData().Replace("\n\r", "|").Split('|');
	long partOne = 0, partTwo = 0;
	foreach(string value in values) {
		Dictionary<char, int> letters = new Dictionary<char, int>();
		int group = 0;
		foreach(string word in value.Split().Where(x => !string.IsNullOrWhiteSpace(x))) {
		...
		partTwo += letters.Values.Count(x => x == group);		
		partOne += letters.Keys.Count();	
	}
	Console.WriteLine($"Part 1: {partOne}");
	Console.WriteLine($"Part 2: {partTwo}");
}	

private string GetSmallData(){
	return @"abc
...
private string GetData(bool bigSet = true) {
	return !bigSet ? GetSmallData() : @"obegcmqadtrhui
qbgocuzeimrhdat
icuagdbztxrqehoy
cuietqhbfroagds
uqdgikwhrvcjeltbpao
